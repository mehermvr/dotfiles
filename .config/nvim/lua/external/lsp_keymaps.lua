...
function M.get()
  local format = function()
    require("external.format").format({ force = true })
  end
  if not M._keys then
  ---@class PluginLspKeys
    -- stylua: ignore
    ...
    if require("external.lazyvim_util").has("inc-rename.nvim") then
      M._keys[#M._keys + 1] = {
        "<leader>cr",
        function()
...
function M.resolve(buffer)
  ...
  local function add(keymap)
    -- if not Keys.active[Keys.parse({ keymap[1], mode = keymap.mode }).id] then
  ...
  local opts = require("external.lazyvim_util").opts("nvim-lspconfig")
  local clients = vim.lsp.get_active_clients({ bufnr = buffer })
  for _, client in ipairs(clients) do
